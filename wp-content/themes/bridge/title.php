					...
					<div class="container">
						<div class="container_inner clearfix">
								...
								<?php } ?>
									...
									<?php } ?>
									<h1 <?php print $page_title_animation_data; if(get_post_meta($id, "qode_page-title-color", true)) { ?> style="color:<?php echo get_post_meta($id, "qode_page-title-color", true) ?>" <?php } ?>><span>
                                                                            <?php
                                                                                $user = wp_get_current_user();
                                                                                if (is_page('profile')) {
                                                                                if ($user->roles[0] == 'company') {
                                                                            ?>
                                                                            Welcome, <?php echo get_user_meta($user->ID,'mepr_company_name',true); ?>
                                                                            <?php } elseif ($user->roles[0] == 'branch') { ?>
                                                                            Welcome, <?php echo get_user_meta($user->ID,'mepr_branch_name',true); ?>
                                                                            <?php } elseif ($user->roles[0] == 'individual' || $user->roles[0] == 'operator' || $user->roles[0] == 'trainer' || $user->roles[0] == 'evaluator') { ?>
                                                                            Welcome, <?php echo $user->display_name; ?>
                                                                            <?php } else { ?>
                                                                            <?php qode_title_text(); ?>
                                                                            <?php } 
                                                                            } else {?>
